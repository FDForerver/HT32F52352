...
/* Private macro -------------------------------------------------------------------------------------------*/
/** @defgroup DAC_Private_Macro DAC private macros
  * @{
  */
 ...
 * @retval None
 ************************************************************************************************************/
void DACD16_SoftwareStartConvCmd(HT_DAC_DUAL16_TypeDef* DACx, DAC_Dual16_Ch DAC_CH_x, ControlStatus NewState)
{
  /* Check the parameters                                                                                   */
  Assert_Param(IS_DAC(DACx));
  Assert_Param(IS_DAC_CHANNEL(DAC_CH_x));
  ...
  if (DAC_CH_x == DAC_CH_R)
  {
    ...
    {
      DACx->TG |= DAC_TRIG_SOFTWARE_RESET << DAC_POS_RCH;
    }
  }
  else
  {
    ...
    {
      DACx->TG |= DAC_TRIG_SOFTWARE_RESET << DAC_POS_LCH;
    }
  }
}
/**
